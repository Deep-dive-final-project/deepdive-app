...
type PostEditorProps = {
  beforePost: PostContent
}

export default function PostEditor({ beforePost }:PostEditorProps) {
  const router = useRouter();
  const { postId } = useParams ();

  const { fetchWithAuth } = useAuth();

  const [title, setTitle] = useState<string>(beforePost.title);
  const [post, setPost] = useState<string>(beforePost.content);
  const [isView, setIsView] = useState<boolean>(false);
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const [summaryMrkdwn, setSummaryMrkdwn] = useState<string>(beforePost.summary);
    ...
    <div className={styles.wrapper}>
      <div className={styles.header}>
        <div className={styles.headerLeft}>
          ...
          <h3 className={styles.headerTitle}>강의 노트 수정</h3>
        </div>
